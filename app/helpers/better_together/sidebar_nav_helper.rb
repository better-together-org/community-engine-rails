      ...
      content_tag :div, class: 'accordion-item' do
        ...
        item_content = content_tag(heading_tag, class: 'accordion-header', id: "heading_#{collapse_id}") do
          if nav_item.children? || nav_item.linkable
            # Use the same link for both the collapse toggle and navigation to the linkable page (using render_page_path)
            link_to render_page_path(nav_item.linkable), class: link_classes, 'data-bs-toggle': 'collapse', 'data-bs-target': "##{collapse_id}", 'aria-expanded': expanded_state, 'aria-controls': collapse_id, 'data-bs-delay-navigation': true do
