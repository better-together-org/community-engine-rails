...
function initializeRichText() {
  ...
  if (!window.headingAttributesAdded) {
    ...
    // Rails' ActiveStorage DirectUpload class dispatches this event on window before
    // each create-blob request fires (event.detail.xhr is the XHR for that specific
    // request). Attach the page-scoped token generated by
    // ApplicationHelper#direct_upload_authorization_token and embedded as a
    // <meta name="direct-upload-token"> tag in every layout, so the server can verify
    // this request came from a page that was actually rendered (see
    // BetterTogether::DirectUploadAuthorization). Covers Trix's built-in attachment
    // flow (which fires this same standard event) and any future
    // file_field(..., direct_upload: true) use, since both go through DirectUpload.
    window.addEventListener("direct-upload:before-blob-request", function(event) {
      const token = document.querySelector('meta[name="direct-upload-token"]')?.content
      if (token) event.detail.xhr.setRequestHeader("X-Direct-Upload-Token", token)
    })
