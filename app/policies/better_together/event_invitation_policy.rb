...
module BetterTogether
  class EventInvitationPolicy < ApplicationPolicy # rubocop:todo Style/Documentation
    def create?
      ...
      # Community members can invite other community members to events in their community
      if record.for_existing_user? && record.invitee && user.person
        same_community?(user.person, record.invitee)
      else
        # For email invitations, require higher permissions
    ...
    def same_community?(person1, person2)
      return false unless person1&.primary_community && person2&.primary_community

      person1.primary_community == person2.primary_community
    end
  end
end
