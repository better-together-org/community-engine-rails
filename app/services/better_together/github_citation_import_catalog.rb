...
module BetterTogether
  ...
  class GithubCitationImportCatalog
    REPOSITORY_LIMIT = 4
    ...
    def sources_for(integration) # rubocop:todo Metrics/MethodLength
    ...
    def build_repository_source(repository)
      {
    ...
    def build_pull_request_sources(github, repository)
      Array(github.pull_requests(repository.full_name, state: 'all', sort: 'updated', direction: 'desc', per_page: PULL_REQUEST_LIMIT)).map do |pull_request|
        {
    ...
    def build_issue_sources(github, repository)
      Array(github.issues_for_repository(repository.full_name, state: 'all', sort: 'updated', direction: 'desc', per_page: ISSUE_LIMIT)).reject do |issue|
    ...
    def build_commit_sources(github, repository)
