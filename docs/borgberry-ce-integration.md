...
graph TD
    subgraph "release/0.11.0-notes — branches merged this session"
        R[copilot/pr-1494<br/>Robot + Agentic<br/>identity helpers]
...
`C3::Token` and `C3::Balance` are intentionally isolated from the `Agentic` / `GovernanceParticipant` models. C3 rewards compute contribution — it does not buy voting power. Governance remains one-member one-vote (co-op principle).
