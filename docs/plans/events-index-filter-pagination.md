...
> Supersedes the earlier draft of this plan (tracking issue #1407) that mapped the
> status filter onto the timing-derived draft/upcoming/ongoing/past scopes; the
> implemented design uses the explicit `events.status` enum instead (see As-built notes).
