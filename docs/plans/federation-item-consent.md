...
- **Per-connection selection matrix** — "federate this item to Platform A but not Platform B."
  The tri-state in this plan is per-item only; a future phase could add a join table
  (`federation_content_grants` or similar) between a content item and a specific
  `PlatformConnection` for exact per-item × per-connection control, if a real need for it
  emerges. Not built now because the simpler tri-state covers the immediate ask.
