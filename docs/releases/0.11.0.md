...
- C3 Tree Seeds community contribution token system — borgberry fleet integration, settlement lifecycle, balance locking, federation token seeds, Joatu peer-exchange layer
- `#1557` — Forgejo C3 settlement and documentation stack preservation
...
- adds a peer-to-peer community contribution token system (Tree Seeds / C3) backed by borgberry fleet-compute earning and volunteer time logging, with a cross-platform federation layer and Joatu peer-exchange integration
...
- **Implemented in `0.11.0`:** federation/platform scoping, privacy/data-rights workflows, community access/review flows, safety reporting, content-security review states, inbound mail ingress, search/reporting hygiene, uploads/content tooling, MCP/API expansion, AI runtime foundations, C3 Tree Seeds token system, short-link share domain, and fleet node authorization
- **Implemented but intentionally gated/documentation-first:** AI runtime/robot configuration
...
## 1A. C3 Tree Seeds — Community Contribution Token System

`0.11.0` ships the first version of the C3 community contribution token system. C3 makes visible the labour and care that holds communities together: members earn Tree Seeds automatically when their devices run borgberry fleet jobs overnight or contribute GPU compute, and manually when community stewards log volunteer time, code review, or moderation work. Seeds can be spent in peer-to-peer exchanges via the Joatu layer already embedded in CE.

**Why it matters:** Community participation is not just governance votes and content posts — it includes compute time, facilitation, peer help, and mutual aid. C3 records and makes exchangeable the contributions that would otherwise be invisible. The borgberry fleet integration means earning is automatic for members who run fleet agents; no manual logging is required for compute contributions.

**Who is affected:** community members earning or spending contribution credits; platform organizers managing the exchange layer and reviewing settlement flows; borgberry fleet operators whose nodes contribute compute; developers integrating C3 federation across connected platforms.

### Major shipped changes

- `C3::Token` model for recording contribution credits with platform scoping and cross-platform origin tracking
- `C3::TokenSeed` STI type for federated token-seed distribution via the federation API (`/api/v1/c3/token_seeds`)
- `Joatu::Settlement` model + `Agreement#fulfill!` — closes the spending chain when an exchange is delivered
- `C3::BalanceLock` with decimal-precision arithmetic and idempotent lock lifecycle; `ExpireBalanceLocksJob` handles automatic expiry of stale locks
- `PlatformConnection` C3 scope for cross-platform token federation with origin tracking as federation prerequisites
- Borgberry CE fleet integration: fleet-node contribution tracking, API endpoints, and autonomous earning registration
- Settlement notifications: `C3::SettlementMailer` + `C3::SettlementNotifier` for operator-owned transaction messaging
- Full i18n: English, Spanish, French, and Ukrainian locale keys for C3 and settlement surfaces

### Governance note

C3 balances have no effect on voting weight. One-member-one-vote is a firm architectural decision: community economic participation (Tree Seeds) is deliberately separated from democratic participation (voting). See `docs/c3/04-regulatory-considerations.md` for the full regulatory treatment.

### Docs coverage

- [What are Tree Seeds?](../c3/00-what-is-c3.md)
- [C3 Data Model](../c3/01-data-model.md)
- [C3 Flows](../c3/02-flows.md)
- [C3 Network and Security](../c3/03-network-and-security.md)
- [C3 Regulatory Considerations](../c3/04-regulatory-considerations.md)
- [Borgberry CE Integration](../borgberry-ce-integration.md)
- [C3 Federation Design](../c3-federation-design.md)
- [C3 Contribution Integration Design](../c3-contribution-integration-design.md)

### Diagram coverage

- architecture diagrams and data model are captured in the `docs/c3/` packet above
- no dedicated release diagram is on this branch beyond the `docs/c3/` documentation packet

### Screenshot coverage

- no dedicated organizer-facing C3 balance or exchange UI ships on this branch tip — the C3 layer is primarily runtime/backend for `0.11.0`
- the Joatu exchange surfaces and balance card are available to authenticated members; dedicated release screenshots are not required for the `0.11.0` packet
- C3 remains intentionally documentation-first for this release; full organizer-facing management screens are deferred to a follow-up cycle
...
- Platform-scoped short-link management view at `GET /c/:community/short_links`
- Public redirect at `GET /r/:slug` — works without authentication
...
- no dedicated diagram required; the routing is straightforward (`/r/:slug` → target URL)
...
- C3 `BalanceLocking`: `lock!` renamed to `lock_c3!` (stops shadowing AR pessimistic locking), error constants qualified, `lock_ref` validated upfront
...
- Devise 5.0.3 security update; Devise 5.0.4 patch released subsequently
- `rack-session` patch update
- `aws-sdk-s3` patch updates (through 1.223.0)
- `addressable` patch update
- ruby_llm 1.15.0
- sidekiq 8.1.5, nokogiri 1.19.3, active_storage_validations 3.0.5
...
- [C3 Tree Seeds documentation packet](../c3/)
- [Borgberry CE Integration](../borgberry-ce-integration.md)
...
- [ ] C3 Tree Seeds section added and linked to `docs/c3/` documentation packet
