...
- **Inconsistently-scoped models — flagged for explicit review, not fixed in this pass:**
  - ~~`C3::Balance`, `C3::Token`, `C3::BalanceLock`~~ — **moot as of this update.** These models, along with the entire C3 Tree Seeds ledger and the Borgberry fleet-compute integration, were fully extracted out of `community-engine-rails` into a standalone `better_together-borgberry` gem (2026-07-28/29, after this assessment's original scoping question was raised on 2026-07-17). Zero C3 code remains in this repo at `release/0.11.0-notes` tip — confirmed via `git grep` against `config/routes.rb`, `app/models/better_together/platform_connection.rb`, and the absence of an `app/models/better_together/c3/` directory entirely. Any cross-platform-scoping decision for the C3 ledger now belongs to that gem's own repo, not here.
  - `PersonLinkedSeed` — uses `belongs_to :source_platform` directly rather than the standard concern.
  - **New in this assessment (found via PR #1581 review):** `Billing::Plan`, `Billing::Subscription`, `Billing::Event` (all added by PR #1581) are also plain `ApplicationRecord`. They're owned polymorphically (`belongs_to :owner`, Community or Person), not platform-scoped — again plausibly intentional (a billing plan catalog might be host-platform-level and offered network-wide), but it's the same "intentional or not, please document the decision" pattern as `PersonLinkedSeed` above.
...
- **Trust model:** `BetterTogether::PlatformConnection` (`app/models/better_together/platform_connection.rb`) — a directed edge between `source_platform` and `target_platform`, with `status` (pending/active/suspended/blocked), `connection_kind` (peer/member), content-sharing/auth policies, and per-scope allow flags (identity, profile_read, content_read, linked_content_read, content_write). *(The C3-token-exchange settings this bullet originally described here were removed along with the rest of the C3/Borgberry extraction — see §4.)*
- **M2M auth:** `app/controllers/better_together/federation/oauth_tokens_controller.rb` — an OAuth2 client_credentials grant scoped to a `PlatformConnection` (routes under `federation/`: `POST federation/oauth/token`, `GET federation/content_feed`, `GET federation/linked_seeds`). Tokens are `BetterTogether::FederationAccessToken` (SHA256-digested, scope-checked, connection-bound). This is **separate from Doorkeeper**, which serves the general JSON:API OAuth2 surface for n8n/MCP/management-tool clients. *(The `federation/c3/token_seeds` and `federation/c3/lock_requests` routes this bullet originally listed no longer exist in this repo — confirmed removed along with the C3/Borgberry extraction.)*
...
- ~~**C3 token federation**~~ — **superseded.** The C3 cross-platform token-portability design this bullet described (`docs/c3-federation-design.md`) belonged to the C3 Tree Seeds system, which has since been fully extracted to the standalone `better_together-borgberry` gem (see §4). Any C3 token-federation work now happens in that gem's own repo, not here; `CHANGELOG.md`'s current "Removed" section documents the relocation.
- **Test coverage** is broad: `spec/requests/better_together/federation/{oauth_tokens,content_feed,linked_seeds}_spec.rb`, `spec/requests/better_together/platform_connections_spec.rb` (26 examples), `spec/services/better_together/federation_scope_authorizer_spec.rb`, `federation_connection_provisioning_service_spec.rb`, `spec/models/better_together/{platform_connection,federation_access_token}_spec.rb`, transport adapter specs. *(The `c3_lock_requests`/`c3_token_seeds` request specs this bullet originally cited were removed along with the C3/Borgberry extraction.)*
