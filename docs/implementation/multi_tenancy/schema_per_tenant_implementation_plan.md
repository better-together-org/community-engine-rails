...
### Decision 3: Planning Treatment Of Unresolved Domains

Domains marked `needs-redesign` in the ownership matrix are explicit prerequisite design items. They are not implicitly assigned during implementation.
...
- resolve `needs-redesign` domains enough to avoid migration ambiguity
- record any remaining exceptions as ADRs
...
The following domains must be resolved before their implementation slices begin:

| Domain | Required Before |
|--------|------------------|
| user identity / authentication boundary | tenant migration and request auth changes |
| content block ownership | content migration and editor behavior updates |
| navigation ownership | tenant-global vs community-local nav refactor |
| conversations / messages | messaging migration and tenant context propagation |
| notifications | delivery storage and tenant/fleet split |
| integrations / OAuth | external auth and integration routing behavior |
| search / analytics | cross-tenant admin strategy and reporting architecture |
>>>>>>> d80dfdfa (chore(agent): checkpoint tenancy planning package)
...
2. Resolve all `needs-redesign` domains required for the first implementation slice.
