...
class AddC3PricingToJoatuOffersRequests < ActiveRecord::Migration[7.2]
  def change
    if table_exists?(:better_together_joatu_offers) && !column_exists?(:better_together_joatu_offers, :c3_price_millitokens)
    ...
    return if column_exists?(:better_together_joatu_requests, :c3_budget_millitokens)
