...
RSpec.feature 'Respond with Offer from Request' do
  ...
  has_hidden_inputs = page.has_selector?("input[name='joatu_offer[response_links_as_response_attributes][0][source_type]'][value='BetterTogether::Joatu::Request']", visible: :hidden) &&
            page.has_selector?("input[name='joatu_offer[response_links_as_response_attributes][0][source_id]'][value='#{request_resource.id}']", visible: :hidden)

  has_new_offer_redirect = current_path == new_offer_path

  has_link_with_params = page.has_css?("a[href*='#{new_offer_path}'][href*='source_type'][href*='#{request_resource.id}']") ||
