...
RSpec.describe 'Event Invitation Token Processing', type: :request do
  let(:locale) { I18n.default_locale }
  let!(:platform) do
    existing = BetterTogether::Platform.find_by(host: true)
    if existing
  ...
  describe 'valid_event_invitation_token_present? helper' do
    ...
    context 'with expired invitation token in session', :skip_host_setup do
