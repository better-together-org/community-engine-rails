...
RSpec.describe 'Enhanced Event Invitation System', type: :request do
  ...
  describe 'platform privacy bypass', :unauthenticated do
    ...
    it 'allows RSVP via invitation on private platform' do
      post better_together.rsvp_going_event_path(public_event.slug, locale: locale, invitation_token: invitation.token)
      expect(response).to have_http_status(:redirect)
      expect(response.location).to include('sign-in')
    end
