...
ActiveRecord::Schema[7.0].define(version: 20_231_125_164_028) do
  ...
  create_table 'better_together_communities', primary_key: 'bt_id', id: :uuid, default: nil, force: :cascade do |t|
    ...
    t.index ['host'], name: 'index_better_together_communities_on_host', unique: true,
                      where: '((host IS TRUE) AND (creator_id IS NULL))'
  ...
  create_table 'better_together_person_community_memberships', primary_key: 'bt_id', id: :uuid, default: nil,
                                                               force: :cascade do |t|
    t.integer 'lock_version', default: 0, null: false
    t.datetime 'created_at', null: false
    t.datetime 'updated_at', null: false
    ...
    t.index %w[community_id member_id role_id], name: 'unique_person_community_membership_member_role',
                                                unique: true
  ...
  add_foreign_key 'better_together_navigation_items', 'better_together_navigation_items', column: 'parent_id',
                                                                                          primary_key: 'bt_id'
  add_foreign_key 'better_together_person_community_memberships', 'better_together_communities',
                  column: 'community_id', primary_key: 'bt_id'
