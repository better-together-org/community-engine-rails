  ...
  it 'captures the hosted entitlement card in grace period' do
    capture_docs_screenshot(
      ...
      callouts: [
        ...
        { selector: '#hosted-entitlement-explainer', title: 'Grace period explainer date',
          bullets: ['Session follow-up: this date used to interpolate a raw ISO timestamp (e.g. "2026-08-23T14:22:00Z") — now renders via l(..., format: :long) like every other date on this page.'] }
      ],
      narrative: {
        ...
        callouts: [
          ...
          { title: 'Grace period explainer date',
            description: 'Second-round remediation follow-up: grace_period_ends_at was previously interpolated with .to_date.to_s, producing a raw, unlocalized ISO date directly in translated copy (this string ships in en/es/fr/uk). It now goes through l(..., format: :long), matching current_period_end and deauthorized_at on the sibling subscription/merchant cards on this same page.' }
        ],
      ...
      expect(page).to have_css('#hosted-entitlement-explainer')
    end
  end
