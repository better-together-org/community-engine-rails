...
# RED-phase acceptance criteria for AC-3.7 (docs/implementation/current_plans/
# event_occurrences_acceptance_criteria.md). Neither the job nor
# Event#next_occurrence_at/#refresh_next_occurrence_at! exist yet — every
# example is expected to fail until Part 3 lands.
RSpec.describe 'BetterTogether::EventNextOccurrenceRefreshScanJob' do
  ...
  it 'advances a recurring event whose stored next_occurrence_at has passed (AC-3.7)' do
    pending 'AC-3.7: EventNextOccurrenceRefreshScanJob not yet implemented'
  ...
  it 'does not touch a recurring event whose next_occurrence_at is already in the future' do
    pending 'AC-3.7: scan job should skip already-fresh rows'
  ...
  it 'does not raise or abort the batch when one record fails to refresh' do
    pending 'AC-3.7: per-record rescue not yet implemented, mirroring EventReminderScanJob'
  ...
  it 'leaves a non-recurring event\'s next_occurrence_at as its own starts_at, untouched by the scan' do
    pending 'AC-3.7: non-recurring events are not scan targets'
