  ...
  describe '#perform' do
    ...
    context 'without shareable' do
      it 'creates a Share record without shareable' do
        expect do
          described_class.new.perform(platform, url, locale, nil, nil, host_platform.id, logged_in)
    ...
    context 'with allowed shareable' do
      ...
      it 'creates a Share record with shareable' do
        expect do
          described_class.new.perform(platform, url, locale, 'BetterTogether::Page', page.id, host_platform.id, logged_in)
        end.to change(BetterTogether::Metrics::Share, :count).by(1)
      end
    ...
    context 'with disallowed shareable type' do
      it 'does not create a Share record' do
        expect do
          described_class.new.perform(platform, url, locale, 'BetterTogether::Person', '123', host_platform.id, logged_in)
    ...
    it 'enqueues the job' do
      expect do
        described_class.perform_later(platform, url, locale, nil, nil, host_platform.id, logged_in)
      end.to have_enqueued_job(described_class).with(platform, url, locale, nil, nil, host_platform.id, logged_in)
    end
