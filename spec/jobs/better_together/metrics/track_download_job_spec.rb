...
RSpec.describe BetterTogether::Metrics::TrackDownloadJob do
  describe '#perform' do
    ...
    it 'creates a Download record' do
      expect do
        described_class.new.perform(page, file_name, file_type, file_size, locale, host_platform.id, logged_in)
    ...
    it 'stores platform and logged-in state' do
      ...
      expect(download.platform).to eq(host_platform)
      expect(download.logged_in).to be(false)
    end

    it 'enqueues the job' do
      expect do
        described_class.perform_later(page, file_name, file_type, file_size, locale, host_platform.id, logged_in)
      end.to have_enqueued_job(described_class).with(page, file_name, file_type, file_size, locale, host_platform.id, logged_in)
    end
