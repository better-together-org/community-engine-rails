...
RSpec.describe BetterTogether::MembershipRequestSubmittedNotifier do
  subject(:notifier) do
    described_class.new(
  ...
  describe '#body' do
    it 'includes the requestor name' do
      expect(notifier.body).to include(membership_request.requestor_name)
    end
  ...
  describe '#build_message' do
    ...
    it 'includes a URL to review the request' do
      message = notifier.build_message(notification)

      expect(message[:url]).to include(community.to_param)
      expect(message[:url]).to include(membership_request.to_param)
    end
  end
  ...
  describe '#email_params' do
    ...
    it 'includes the membership_request' do
      params = notifier.email_params(notification)

      expect(params[:membership_request]).to eq(membership_request)
    end

    it 'includes a review_url' do
      params = notifier.email_params(notification)

      expect(params[:review_url]).to be_present
    end
