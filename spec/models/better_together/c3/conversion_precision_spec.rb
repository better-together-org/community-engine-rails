...
RSpec.describe 'BetterTogether::C3::Token conversion precision' do
  ...
  describe '.c3_to_millitokens' do
    context 'with valid C3 amounts' do
      it 'converts whole numbers correctly' do
        ...
        expect(BetterTogether::C3::Token.c3_to_millitokens(1)).to eq(1_000)
        expect(BetterTogether::C3::Token.c3_to_millitokens(10)).to eq(10_000)
        expect(BetterTogether::C3::Token.c3_to_millitokens(100)).to eq(100_000)
      end

      it 'converts fractional amounts correctly' do
        # 0.1 Tree Seeds = 100 millitokens
        expect(BetterTogether::C3::Token.c3_to_millitokens(0.1)).to eq(100)
        # 0.5 Tree Seeds = 500 millitokens
        expect(BetterTogether::C3::Token.c3_to_millitokens(0.5)).to eq(500)
        # 0.001 Tree Seeds = 1 millitoken (minimum unit)
        expect(BetterTogether::C3::Token.c3_to_millitokens(0.001)).to eq(1)
      end

      it 'converts the minimum unit (0.001 Tree Seeds = 1 millitoken)' do
        result = BetterTogether::C3::Token.c3_to_millitokens('0.001')
        expect(result).to eq(1)
      end

      it 'converts common practical amounts' do
        # Common contribution amounts
        expect(BetterTogether::C3::Token.c3_to_millitokens(1.5)).to eq(1_500)
        expect(BetterTogether::C3::Token.c3_to_millitokens(2.25)).to eq(2_250)
        expect(BetterTogether::C3::Token.c3_to_millitokens(10.75)).to eq(10_750)
      end
      ...
      it 'handles string input correctly' do
        ...
        expect(BetterTogether::C3::Token.c3_to_millitokens('1')).to eq(1_000)
        expect(BetterTogether::C3::Token.c3_to_millitokens('0.5')).to eq(500)
        expect(BetterTogether::C3::Token.c3_to_millitokens('1.875')).to eq(1_875)
      end

      it 'handles zero' do
        expect(BetterTogether::C3::Token.c3_to_millitokens(0)).to eq(0)
        expect(BetterTogether::C3::Token.c3_to_millitokens('0')).to eq(0)
      end
    end
  ...
  describe '.millitokens_to_c3' do
    context 'with various millitoken amounts' do
      it 'converts whole millitoken values to C3' do
        expect(BetterTogether::C3::Token.millitokens_to_c3(1_000)).to eq(1.0)
        expect(BetterTogether::C3::Token.millitokens_to_c3(10_000)).to eq(10.0)
        expect(BetterTogether::C3::Token.millitokens_to_c3(100_000)).to eq(100.0)
      end

      it 'converts partial millitokens correctly' do
        expect(BetterTogether::C3::Token.millitokens_to_c3(500)).to eq(0.5)
        expect(BetterTogether::C3::Token.millitokens_to_c3(100)).to eq(0.1)
        expect(BetterTogether::C3::Token.millitokens_to_c3(1)).to eq(0.001)
      end

      it 'rounds results to 4 decimal places' do
        # 1234 millitokens = 1.234 C3 (exact)
        expect(BetterTogether::C3::Token.millitokens_to_c3(1_234)).to eq(1.234)
        # 1567 millitokens = 1.567 C3 (exact)
        expect(BetterTogether::C3::Token.millitokens_to_c3(1_567)).to eq(1.567)
      end

      it 'handles zero' do
        expect(BetterTogether::C3::Token.millitokens_to_c3(0)).to eq(0.0)
      end
    end
  end
  ...
  describe '.millitokens_to_c3_decimal' do
    it 'returns a BigDecimal for precise API responses' do
      result = BetterTogether::C3::Token.millitokens_to_c3_decimal(1_000)
      expect(result).to be_a(BigDecimal)
      expect(result).to eq(BigDecimal('1'))
    end

    it 'maintains precision in decimal results' do
      result = BetterTogether::C3::Token.millitokens_to_c3_decimal(1_234)
  ...
  describe 'round-trip conversion accuracy' do
    it 'maintains precision for whole numbers' do
      original_c3 = 10
      millitokens = BetterTogether::C3::Token.c3_to_millitokens(original_c3)
      converted_back = BetterTogether::C3::Token.millitokens_to_c3(millitokens)
    ...
    it 'maintains precision for decimal amounts with trailing zeros' do
      ...
      millitokens = BetterTogether::C3::Token.c3_to_millitokens(original_c3_str)
      converted_back = BetterTogether::C3::Token.millitokens_to_c3(millitokens)
  ...
  describe 'accumulation accuracy' do
    it 'accumulates common contributions without precision loss' do
      ...
      total_millitokens = contributions.sum { |amount| BetterTogether::C3::Token.c3_to_millitokens(amount) }
      ...
      total_c3 = BetterTogether::C3::Token.millitokens_to_c3(total_millitokens)
    ...
    it 'accumulates fractional amounts correctly' do
      ...
      total_millitokens = amounts.sum { |amount| BetterTogether::C3::Token.c3_to_millitokens(amount) }
    ...
    it 'handles 100 small accumulations without error' do
      ...
      total_millitokens = 100.times.sum { BetterTogether::C3::Token.c3_to_millitokens(0.01) }
  ...
  describe 'Token model with safe conversions' do
    it 'sets c3_amount using the safe conversion method' do
      token = BetterTogether::C3::Token.new
      token.c3_amount = 1.5
  ...
  describe 'Edge cases and boundary conditions' do
    it 'handles very large amounts correctly' do
      ...
      millitokens = BetterTogether::C3::Token.c3_to_millitokens(large_amount)
    ...
    it 'converts the maximum 4-decimal precision value' do
      ...
      millitokens = BetterTogether::C3::Token.c3_to_millitokens('0.9999')
    ...
    it 'does not lose precision with repeated conversions' do
      ...
      millitokens = BetterTogether::C3::Token.c3_to_millitokens(original)
      result1 = BetterTogether::C3::Token.millitokens_to_c3(millitokens)

      # Convert again to millitokens from the result
      millitokens2 = BetterTogether::C3::Token.c3_to_millitokens(result1)
      result2 = BetterTogether::C3::Token.millitokens_to_c3(millitokens2)
