...
RSpec.describe BetterTogether::EventInvitation, type: :model do
  ...
  describe 'enhanced scopes' do
    ...
    describe '.for_existing_users' do
      it 'returns only invitations with an invitee person' do
        results = BetterTogether::EventInvitation.for_existing_users
    ...
    describe '.for_email_addresses' do
      it 'returns only invitations with an email address but no invitee' do
        results = BetterTogether::EventInvitation.for_email_addresses
  ...
  describe 'enhanced acceptance flow' do
    ...
    describe '#after_accept!' do
      ...
      it 'ensures community membership for the invitee' do
        expect do
          invitation.after_accept!(invitee_person: invitee_person)
        end.to change { community.person_community_memberships.count }.by(1)

        membership = community.person_community_memberships.find_by(member: invitee_person)
        expect(membership).to be_present
        expect(membership.role.identifier).to eq('community_member')
      end
  ...
  describe 'token generation' do
    it 'automatically generates a token before validation' do
      ...
      expect(invitation.token.length).to eq(32) # SecureRandom.hex(16) produces 32 characters
    end
