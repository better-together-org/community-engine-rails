...
RSpec.describe BetterTogether::SocialMediaAccount do
  describe 'factory' do
  ...
  describe 'validations' do
    ...
    it { is_expected.to validate_inclusion_of(:platform_name).in_array(described_class::PLATFORMS) }
    ...
    it 'validates uniqueness of platform scoped to contact_detail' do
      ...
      create(:social_media_account, contact_detail: contact_detail, platform_name: 'Facebook')
      duplicate = build(:social_media_account, contact_detail: contact_detail, platform_name: 'Facebook')

      expect(duplicate).not_to be_valid
      expect(duplicate.errors[:platform_name]).to include('account already exists for this contact')
    end

    it 'allows same platform for different contact_details' do
      contact1 = create(:contact_detail)
      contact2 = create(:contact_detail)
      create(:social_media_account, contact_detail: contact1, platform_name: 'Facebook')
      account2 = build(:social_media_account, contact_detail: contact2, platform_name: 'Facebook')
  ...
  describe 'url generation' do
    ...
    it 'does not override existing url' do
      ...
      account = create(:social_media_account, platform_name: 'Facebook', handle: 'johndoe', url: existing_url)
    ...
    it 'regenerates url when platform changes with handle present and url blank' do
      account = create(:social_media_account, platform_name: 'Facebook', handle: 'johndoe', url: nil)
      account.update(platform_name: 'Instagram', url: nil)
      expect(account.url).to eq('https://www.instagram.com/johndoe')
    end
  end
  ...
  describe 'handle sanitization' do
    ...
    it 'parameterizes handle' do
      account = create(:social_media_account, platform_name: 'Facebook', handle: 'John Doe', url: nil)
  ...
  describe '#to_s' do
    it 'returns platform and handle' do
      account = build(:social_media_account, platform_name: 'Facebook', handle: 'johndoe')
