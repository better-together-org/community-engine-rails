...
RSpec.describe BetterTogether::Safety::ActionPolicy do
  ...
  let(:action_record) { BetterTogether::Safety::Action.new(safety_case:, actor: safety_reviewer.person) }

  before do
    grant_platform_permission(safety_reviewer, 'manage_platform_safety')
  end

  it 'permits explicit safety reviewers to create actions' do
