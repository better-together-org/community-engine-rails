...
RSpec.describe BetterTogether::ContentSecurity::MailScreeningService do
  ...
  it 'syncs a ContentSecurity::Subject so held mail is visible in the review queue' do
    community = create(:better_together_community, name: 'Screening Subject Sync')
    platform = create_tenant(community:, domain: 'tenant-subject-sync.example.test')
    inbound_email = build_inbound_email(raw_mail(to: "community+#{community.slug}@tenant-subject-sync.example.test"))
    message = build(
      :inbound_email_message,
      inbound_email:,
      platform:,
      target: community,
      recipient_address: "community+#{community.slug}@tenant-subject-sync.example.test",
      recipient_local_part: "community+#{community.slug}",
      recipient_domain: 'tenant-subject-sync.example.test'
    )
    resolution = BetterTogether::InboundEmailResolutionService::Resolution.new(
      'community',
      community,
    ...
    review_subject = BetterTogether::ContentSecurity::Subject.find_by(subject: message)
    expect(review_subject).to be_present
    expect(BetterTogether::ContentSecurity::Subject.review_queue).to include(review_subject)
  end
end
