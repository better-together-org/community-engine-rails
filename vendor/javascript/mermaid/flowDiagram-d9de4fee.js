import { p as kt, f as W } from "./flowDb-5aa2917a.js";
import { j as x, n as G, o as z, p as Z, c as D, r as tt, f as et, l as M, q as $, t as St } from "./mermaid-b92f6f74.js";
import { G as Lt } from "./graph-80608c14.js";
import { u as Et, r as _t, p as Tt, l as Nt, d as I } from "./layout-0254eb32.js";
import { a as T, b as rt, i as at, c as E, e as nt, d as st, s as At, f as Pt, g as Ct } from "./styles-f321baca.js";
import { l as It } from "./line-0c78f995.js";
...
function q(r, e, t) {
  ...
  e.labelType === "svg" ? $t(a, e) : typeof n != "string" || e.labelType === "html" ? rt(a, e) : zt(a, e);
...
function ae(r, e, t, n) {
  var a, s, i, o, c, d, l, y, h, u, f, v, p, g, k;
  if (a = e.y - r.y, i = r.x - e.x, c = e.x * r.y - r.x * e.y, h = a * t.x + i * t.y + c, u = a * n.x + i * n.y + c, !(h !== 0 && u !== 0 && J(h, u)) && (s = n.y - t.y, o = t.x - n.x, d = n.x * t.y - t.x * n.y, l = s * r.x + o * r.y + d, y = s * e.x + o * e.y + d, !(l !== 0 && y !== 0 && J(l, y)) && (f = a * o - s * i, f !== 0)))
    return v = Math.abs(f / 2), p = i * d - o * c, g = p < 0 ? (p - v) / f : (p + v) / f, p = s * c - a * d, k = p < 0 ? (p - v) / f : (p + v) / f, { x: g, y: k };
}
function J(r, e) {
  return r * e > 0;
}
function _(r, e, t) {
  ...
  return s.length ? (s.length > 1 && s.sort(function(f, v) {
    var p = f.x - t.x, g = f.y - t.y, k = Math.sqrt(p * p + g * g), P = v.x - t.x, L = v.y - t.y, O = Math.sqrt(P * P + L * L);
...
function fe(r) {
  r.shapes().question = lt, r.shapes().hexagon = ct, r.shapes().stadium = gt, r.shapes().subroutine = vt, r.shapes().cylinder = wt, r.shapes().rect_left_inv_arrow = dt, r.shapes().lean_right = ht, r.shapes().lean_left = ut, r.shapes().trapezoid = pt, r.shapes().inv_trapezoid = ft, r.shapes().rect_right_inv_arrow = yt;
}
function ye(r) {
  r({ question: lt }), r({ hexagon: ct }), r({ stadium: gt }), r({ subroutine: vt }), r({ cylinder: wt }), r({ rect_left_inv_arrow: dt }), r({ lean_right: ht }), r({ lean_left: ut }), r({ trapezoid: pt }), r({ inv_trapezoid: ft }), r({ rect_right_inv_arrow: yt });
}
function N(r, e, t, n) {
...
const ge = {
  ...
  for (const d of c) {
    ...
    if (Z(D().flowchart.htmlLabels)) {
      ...
      f = rt(i, g).node(), f.parentNode.removeChild(f);
    } else {
      ...
      for (const P of k) {
        ...
        L.setAttributeNS("http://www.w3.org/XML/1998/namespace", "xml:space", "preserve"), L.setAttribute("dy", "1em"), L.setAttribute("x", "1"), L.textContent = P, g.appendChild(L);
      }
      f = g;
    }
    let v = 0, p = "";
  ...
  for (const i of r) {
    ...
    l.style = y, l.labelStyle = h, i.interpolate !== void 0 ? l.curve = $(i.interpolate, G) : r.defaultInterpolate !== void 0 ? l.curve = $(r.defaultInterpolate, G) : l.curve = $(mt.curve, G), i.text === void 0 ? i.style !== void 0 && (l.arrowheadStyle = "fill: #333") : (l.arrowheadStyle = "fill: #333", l.labelpos = "c", Z(D().flowchart.htmlLabels) ? (l.labelType = "html", l.label = `<span id="L-${o}" class="edgeLabel L-${c}' L-${d}" style="${l.labelStyle}">${await tt(
      i.text.replace(
        /fa[blrs]?:fa-[\w-]+/g,
        // cspell:disable-line
...
`), i.style === void 0 && (l.style = l.style || "stroke: #333; stroke-width: 1.5px;fill:none"), l.labelStyle = l.labelStyle.replace("color:", "fill:"))), l.id = o, l.class = c + " " + d, l.minlen = i.length || 1, e.setEdge(t.db.lookUpDomId(i.start), t.db.lookUpDomId(i.end), l, n);
  ...
  M.warn("Get vertices", v);
  const p = n.db.getEdges();
  let g = 0;
  for (g = f.length - 1; g >= 0; g--) {
    u = f[g], At("cluster").append("text");
  ...
  await xt(v, h, e, o, c, n), await bt(p, h, n);
  const k = new ce();
  ge.addToRender(k), k.arrows().none = function(b, S, m, C) {
  ...
  }), n.db.indexNodes("subGraph" + g), g = 0; g < f.length; g++)
    if (u = f[g], u.title !== "undefined") {
  ...
  St(h, P, s.diagramPadding, s.useMaxWidth), Object.keys(v).forEach(function(w) {
    const b = v[w];
    if (b.link) {
  ...
  styles: Ct,
  init: (r) => {
    r.flowchart || (r.flowchart = {}), r.flowchart.arrowMarkerAbsolute = r.arrowMarkerAbsolute, xe.setConf(r.flowchart), W.clear(), W.setGen("gen-1");
  }
};
export {
  Me as diagram
